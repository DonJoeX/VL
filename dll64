...
echo "C:\Program Files (x86)\Common Files\SYSTEM\ADO\msado15.dll" >> C:\TSS\dll64.txt
GOTO ESITO 
...
echo "C:\Program Files (x86)\Common Files\microsoft shared\DAO\dao360.dll" >> C:\TSS\dll64.txt
GOTO ESITO 
...
IF %errorlevel% EQU 0 (echo Registrazione riuscita.
	echo Registrazione riuscita. >> C:\TSS\dll64.txt
	GOTO N
) ELSE IF %errorlevel% EQU 5 (color 0E
	echo Errore: Accesso negato. Esegui come amministratore.
	echo Errore: Accesso negato. Esegui come amministratore. >> C:\TSS\dll64.txt
	GOTO OPZIONI
) ELSE IF %errorlevel% EQU 3 (color 0C
	echo Errore: File non trovato.
	echo Errore: File non trovato. >> C:\TSS\dll64.txt
	GOTO OPZIONI
) ELSE (color 0D
	echo Errore sconosciuto. Codice: %errorlevel%
	echo Errore sconosciuto. Codice: %errorlevel% >> C:\TSS\dll64.txt
	GOTO OPZIONI
)
...
:END
echo "Il file con il risultato dell'operazione e' stato salvato (C:\TSS\dll64.txt)"
pause
